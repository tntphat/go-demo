...
func GetLocationFromIP(ip string, apiKey string) (*serializers.IPResp, error) {
	resp, err := http.Get("http://api.ipstack.com/" + ip + "?access_key=" + apiKey)
	if err != nil {
		return nil, err
	}
	var sResp serializers.IPResp
